...
WORKSPACE_PATH=`( cd "$SINGULARITY_PATH/../." && pwd )`
...
MOUNTS=(
  ...
  "type=bind" "$WORKSPACE_PATH" "/home/$USER/ros_ws"

  # mount the MRS shell additions into the container, DO NOT MODIFY
  "type=bind" "$MOUNT_PATH" "/opt/host"
)
...
if $CONTAINED; then
  CONTAINED_ARG="--home /tmp/singularity/$FILE_NAME/home:/home/$USER"
...
if $DETACH_TMP; then
  TMP_PATH="/tmp/singularity/$FILE_NAME/tmp"
...
[ ! -e /tmp/singularity/$FILE_NAME/tmp ] && mkdir -p /tmp/singularity/$FILE_NAME/tmp
[ ! -e /tmp/singularity/$FILE_NAME/home ] && mkdir -p /tmp/singularity/$FILE_NAME/home
